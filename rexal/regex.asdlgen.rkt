...
(define MARSHAL#PRODUCT-TYPE?
  (lambda (T)
    (let ((mod (%GET-TYPE->MODULE T))
          (param/count (%GET-TYPEPROD->PARAMETER-LIST-COUNT T))
          (param/types (%GET-TYPEPROD->PARAMETER-LIST-TYPES T))
          (param/ids (%GET-TYPEPROD->PARAMETER-LIST-IDS T))
          (param/tyop (%GET-TYPEPROD->PARAMETER-LIST-TYPE-OPT T)))
      (let ((args (MARSHAL#PARAMETERS param/count param/ids)))
        `(define ,(f/module.type? mod T)
          (lambda (obj)
            (let ((self (lambda _ ',T)))
  ...
  (lambda (types tyop args)
    ...
    (BLOCK
     (map
      (lambda (t o a)
        (let ((mod (%GET-TYPE->MODULE t)))
          (list (select/type/check/function o) 'self
                (f/module.type? mod t) a)))
      types
      tyop
      args ) ) ) )
    ...
    (let (($type (%GET-CONSTRUCTOR->TYPE constructor))
      ...
      `(define ,$consid
        (lambda ,$params
          ...
          ;; (define $konsid  (lambda (self) (lambda () ,$consid)))
          ;; (define $type  (lambda (self) (lambda () TYPE)))
...
(define ITERATOR#TYPE-NAME-LIST
  (lambda (sum product)
    (for-each
     (lambda (T)
       (if (eq? 'TYPE-SUM (%GET-TYPE->KIND T))
           (sum T)
           (if (eq? 'TYPE-PRODUCT (%GET-TYPE->KIND T))
               (product T)
...
(define EMIT#TYPE-PRODUCT
  (lambda (T)
    (newline)
    (display ";;; ")
    (display T)
    (display " -- product type constructed by (cons)")
    (newline)
    (display (f/pretty (MARSHAL#PRODUCT-TYPE? T)))
    (newline)
    ))

(define EMIT#CONSTRUCTOR
  (lambda (constructor)
    (display (f/pretty (MARSHAL#CONSTRUCTOR constructor)))
    (newline)
    (for-each
     (lambda (s)
       (display (f/pretty s))
